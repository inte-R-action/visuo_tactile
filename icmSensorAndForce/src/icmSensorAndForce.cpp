...
{
	ROS_INFO("Pressure sensor data received: [%f]", msg->data);
...
int main(int argc, char** argv)
{
   ...
   if( argc < 3 )
    ...
    std::string folderPath = argv[1];       // folder path to store the data
    std::string expNumber = argv[2];		// experiment number
    int numOfRows = std::stoi(argv[3]);		// number of rows (vertical movements) for exploration of the sensor
    int numOfColumns = std::stoi(argv[4]); 	// number of columns (horizontal movements) for exploration of the sensor
    float desiredContactForce = std::stof(argv[5]);	// desired force to touch the sensor with the probe
    float maxContactForce = desiredContactForce*1.10;	// maximum force to touch the sensor (10% tolerance) = desired forece + 10%


    cout << "===========================================================" << endl;
    cout << "EXPERIMENT INFORMATION" << endl;
    cout << "Folder path and name: " << folderPath << endl;
    cout << "Experiment number: " << expNumber << endl;
    cout << "[Rows, Columns] : [" << numOfRows << ", " << numOfColumns << "]" << endl;
    cout << "Desired contact force: " << desiredContactForce << endl;
    cout << "Maximum contact force: " << maxContactForce << endl;
    cout << "===========================================================" << endl;

    cin;

    ros::init(argc, argv, "icmSensorAndForce");
    ros::NodeHandle node_handle;
    ros::AsyncSpinner spinner(1);
    spinner.start();


	ros::Subscriber subPressure = node_handle.subscribe("pressure", 1000, sensorDataCallback);
...
//    robot_state::RobotStatePtr kinematic_state = robot_state::RobotStatePtr(move_group.getCurrentState());
    ...
    float robotTouchZ = 0.034;	// 10mm it was 16mm (0.016)
    ...
    float incStepZ = -0.0001;	// 1mm movement down
...
//        target_pose3.position.y = target_pose3.position.y - yHomePosition[k];
        ...
        ROS_INFO_NAMED("tutorial", "Moving left/right (Cartesian path) (%.2f%% acheived)", fraction * 100.0);
...
//			myfile.open (folderPath + "/icmSensorData_exp_" + expNumber + "r" + j + "c" + k + ".txt");
	        ...
	        cout << "Position for touch [row, col] = [" << j << ", " << k << "]" << endl;
...
//	            move_group.setStartState(*move_group.getCurrentState());
       		    // Gets the current pose of the robot
//           		geometry_msgs::Pose target_poseInitForce = homeZPosition;
	            // Stores the first target pose or waypoint
//	            geometry_msgs::Pose target_pose4 = target_poseInitForce;

            	target_pose3.position.z = homeZPosition.position.z;

		        float maxTouchZPosition = target_pose3.position.z + zPosition[i];

		        cout << "Init touch Z position = " <<  target_pose3.position.z << endl;
		        cout << "Max touch Z position = " <<  maxTouchZPosition << endl;

//		        sleep(0.1);

//            	while( (sensorForce < desiredContactForce) || (target_pose4.position.z < zPosition[i]) )
            	while( (target_pose3.position.z > maxTouchZPosition) && (sensorForce < desiredContactForce) )
            	{      
		            move_group.setStartState(*move_group.getCurrentState());
		            target_poseInit = move_group.getCurrentPose().pose;
					target_pose3 = target_poseInit;
		            // Vector to store the waypoints for the planning process
		            std::vector<geometry_msgs::Pose> waypointsForce;

		            target_pose3.position.z = target_pose3.position.z + incStepZ;
		            waypointsForce.push_back(target_pose3);

			        cout << "Target touch Z position = " <<  target_pose3.position.z << endl;
//			        sleep(0.1);

		            moveit_msgs::RobotTrajectory trajectory;
		            const double jump_threshold = 0.0;
		            const double eef_step = 0.001;
		            double fraction = move_group.computeCartesianPath(waypointsForce, eef_step, jump_threshold, trajectory, true);
		            ROS_INFO_NAMED("tutorial", "Moving down (Cartesian path) (%.2f%% acheived)", fraction * 100.0);

		            // Visualize the plan in RViz
		            visual_tools.deleteAllMarkers();
		            visual_tools.publishText(text_pose, "Cartesian Path", rvt::WHITE, rvt::XLARGE);
		            visual_tools.publishPath(waypointsForce, rvt::LIME_GREEN, rvt::SMALL);
		            for (std::size_t iterVis = 0; iterVis < waypointsForce.size(); ++iterVis)
		            visual_tools.publishAxisLabeled(waypointsForce[iterVis], "pt" + std::to_string(iterVis), rvt::SMALL);
		            visual_tools.trigger();
		            ...
		            if( sensorForce < desiredContactForce )
		            {
		            // You can execute a trajectory like this.
		            move_group.execute(trajectory);
		            ...
		            cout << "Reached touch Z position = " << target_pose3.position.z << endl;
		        	}
//		            sleep(0.1);
            	}
            	cout << "::: Sensor force: " << sensorForce << " ---- Position: " << target_pose3.position.z << endl;
	            sleep(3);
            }
            else if( i == 1 )
			{
            ...
            const double eef_step = 0.01;
            double fraction = move_group.computeCartesianPath(waypoints, eef_step, jump_threshold, trajectory, true);
            ROS_INFO_NAMED("tutorial", "Moving up (Cartesian path) (%.2f%% acheived)", fraction * 100.0);

            // Visualize the plan in RViz
            visual_tools.deleteAllMarkers();
            visual_tools.publishText(text_pose, "Cartesian Path", rvt::WHITE, rvt::XLARGE);
            visual_tools.publishPath(waypoints, rvt::LIME_GREEN, rvt::SMALL);
            for (std::size_t iterVis = 0; iterVis < waypoints.size(); ++iterVis)
            visual_tools.publishAxisLabeled(waypoints[iterVis], "pt" + std::to_string(iterVis), rvt::SMALL);
            visual_tools.trigger();
            ...
            const double eef_step = 0.01;
            double fraction = move_group.computeCartesianPath(waypoints, eef_step, jump_threshold, trajectory, true);
            ROS_INFO_NAMED("tutorial", "Moving back (Cartesian path) (%.2f%% acheived)", fraction * 100.0);
